% Analyze three very different cancer types:   5    13    30
load('PANCANDigitLabels.mat'); Labels = PANCANDigitLabels;
load('PANCANFeatures.mat')

%5=breast, 13=kidney, 16=lung, 30=thyroid

% Choose very different cancer types:
index5 = find(PANCANDigitLabels==5); Labels(index5)=1;
index13 = find(PANCANDigitLabels==13); Labels(index13)=2;
index30 = find(PANCANDigitLabels==30); Labels(index30)=3;
SubsetIndex = [index5; index13; index30];
...
mu(3,:) = mean(PANCANFeatures(index30,GenesToKeep)); 
...
CenteredX3 = PANCANFeatures(index30,GenesToKeep) - ones(length(index30),1)*(mu(3,:));
