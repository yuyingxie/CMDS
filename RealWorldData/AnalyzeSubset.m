...
SubsetIndex = [index5; index13; index16; index30];
...
load('KIRC_p_vals_QN.mat');
load('BRCA_p_vals_QN.mat');
load('LUAD_p_vals_QN.mat');
load('THCA_p_vals_QN.mat');
MinVar = min([Var5; Var13; Var16; Var30]);
max_p_vals = max([BRCA_p_vals_QN; THCA_p_vals_QN; LUAD_p_vals_QN; KIRC_p_vals_QN]);
%GenesToKeep = find(MinVar>0 & max_p_vals<.00212);
%GenesToKeep = find(MinVar>0 & max_p_vals<.00212); %Keep top 5000, no QN
%GenesToKeep = find(MinVar>0 & max_p_vals<.000000001); %about 1000 genes,
%no QN
...
%% Run k-means on full data set
 ...
 %% Run SLC on full data set 
 %Z2 = linkage(denoisedX,'complete');
 Z2 = linkage(denoisedX,'average');
 %Z2 = linkage(denoisedX,'single');
 SL_clusteringAllData = cluster(Z2,'MaxClust',k);
 
 %% Quantify Accuracy of SLC on full data
 M3 = confusionmat(denoisedLabels, SL_clusteringAllData);
