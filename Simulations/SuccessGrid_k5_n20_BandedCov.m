...
d=floor(exp(linspace(log(100),log(15000),p)));
m = 10; %number of sigma values
sigmasq = exp(linspace(-7,-5.4,m));
sigma = sqrt(sigmasq);
...
for s=1:p
    ...
    for q = 1:m
        ...
        for i=1:J
            X = zeros(N,d(s));
            Cov = full(MakeBandedCov(d(s),5,.5));
            SqrtCov = Cov^(1/2);
        ...
        save('SuccessGrid_k5_n20_BandedCov.mat')
    end
end
