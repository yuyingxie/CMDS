...
n=floor(exp(exp(linspace(1.9,log(log(27000)),10))));
%n=floor(exp(exp(1.9)));
p=size(n,2);
d = 10;  %Regime 1: fixed d
Cov = full(MakeBandedCov(d,5,.5));
SqrtCov = Cov^(1/2);
sigmasq = exp(linspace(-164/norm(Cov),-166.47/norm(Cov),10));
%sigmasq = exp(-165/norm(Cov));
sigma = sqrt(sigmasq);
m=size(sigma,2);
J = 30; %Number of simulations
...
%              end 
        ...
        save('SuccessGrid_k4_d10_BandedCov.mat')
    end
end
