...
for s=1:p
    ...
    for q = 1:m
        Failure = zeros(1,J);
        EffDimGridEstimates = zeros(1,J);
        for i=1:J
            X = zeros(N,d(s));
            Cov = MakeKNNCov(d(s),K,a);
            [VC, DC] = eig(Cov);
            NormCov = max(abs(diag(DC)));
            EffDimGridEstimates(i) = sum(abs(diag(DC)).^2)/NormCov^2;
            Cov = Cov./NormCov;
            SqrtCov = Cov^(1/2);
            for j=1:k
        ...
        save('SuccessGrid_k5_n20_KNNCovNormalized_K10.mat')
    end
end
